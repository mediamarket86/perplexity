...
<div class="catalog-item-info">
	...
	<div class="item-price-cont<?=(!$inOldPrice && !$inPercentPrice ? ' one' : '').(($inOldPrice && !$inPercentPrice) || (!$inOldPrice && $inPercentPrice) ? ' two' : '').($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"]) ? ' reference' : '');?>" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
		<?//TOTAL_OFFERS_PRICE//
		if($haveOffers) {
			...
			<?} else {?>
				<div class="item-price">
					...
					<span class="catalog-item-price">
						...
						<?}
						echo $arElement["MIN_PRICE"]["RATIO_PRICE"];?>
						<span class="unit">
							<?=$currency?>
							<span><?=Loc::getMessage("CT_BCS_ELEMENT_UNIT")." ".(($inPriceRatio) ? $arElement["CATALOG_MEASURE_RATIO"] : "1")." ".$arElement["CATALOG_MEASURE_NAME"];?></span>
						</span>
	...
	if($haveOffers) {
		...
		<?if(is_array($arElement["PRICE_MATRIX_SHOW"]["COLS"]) && count($arElement["PRICE_MATRIX_SHOW"]["COLS"]) > 1) {?>
			<div class="catalog-price-ranges">
						...
						<?}?>
							...
							<?if(!isset($arElement["SELECT_PROPS"]) || empty($arElement["SELECT_PROPS"])) {?>
								...
								<?if(!empty($arElement["PROPERTIES"]["CML2_ARTICLE"]["VALUE"])) {
									$props = array();
									$props[] = array(
										"NAME" => $arElement["PROPERTIES"]["CML2_ARTICLE"]["NAME"],
										"CODE" => $arElement["PROPERTIES"]["CML2_ARTICLE"]["CODE"],
	...
	<?	//OFFERS_DELAY//
		if($haveOffers) {								
			if($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["CAN_BUY"] && $arElement["TOTAL_OFFERS"]["MIN_PRICE"]["RATIO_PRICE"] > 0) {
				$props = array();
				if(!empty($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["PROPERTIES"]["CML2_ARTICLE"]["VALUE"])) {
					$props[] = array(
						"NAME" => $arElement["TOTAL_OFFERS"]["MIN_PRICE"]["PROPERTIES"]["CML2_ARTICLE"]["NAME"],
						"CODE" => $arElement["TOTAL_OFFERS"]["MIN_PRICE"]["PROPERTIES"]["CML2_ARTICLE"]["CODE"],
						"VALUE" => $arElement["TOTAL_OFFERS"]["MIN_PRICE"]["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]
					);																
				}
    ...
    <div class="hidden_for_popup">
        <a href="javascript:void(0)" class="minus_popup" id="quantity_minus_popup_<?=$itemIds['ID']?>"><span>-</span></a>
        <input type="text" id="quantity_popup_<?=$itemIds['ID']?>" name="quantity_popup" class="quantity_popup" value="<?= $arElement['MIN_PRICE']['MIN_QUANTITY']?>"/>
        <a href="javascript:void(0)" class="plus_popup" id="quantity_plus_popup_<?=$itemIds['ID']?>"><span>+</span></a>
    </div>
</div>