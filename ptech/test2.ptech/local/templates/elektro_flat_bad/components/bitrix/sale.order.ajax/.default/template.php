	...
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="bx-soa-order">
			...
			<div class="bx-soa-mainbar">
                ...
                <div id="bx-soa-orderComment" class="bx-soa-order-comment">
                </div>

				<?//AGREEMENT//
				if($arSetting["SHOW_PERSONAL_DATA"]["VALUE"] == "Y" && ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y")) {?>
				...
				<div id="bx-soa-orderSave" class="bx-soa-order-save">
                    ...
                    <?php
                    /**
                     * Формы конфликтуют, сюда вынес кнопку, а сам компонент внизу
                     * костылями дублируем кнопку в мобильный вид
                     */
                    ?>
                    ...
                    <script>
                        BX.ready(function () {
                            $('body').on('click', '.pvp-order-exportxls-block.ext-btn .download-btn', function () {
                                $('.order-page-download-form  .export-form').submit();
                            });

                            let elm = $('.pvp-order-exportxls-block.ext-btn').clone();

                            elm.appendTo('.bx-soa-cart-total-button-container.hidden')
                        });
                    </script>
...
<div class="order-page-download-form">
