...
if(array_key_exists("PROPERTIES", $arResult) && is_array($arResult["PROPERTIES"])) {
	...
	if(array_key_exists("SALELEADER", $arResult["PROPERTIES"]) && !$arResult["PROPERTIES"]["SALELEADER"]["VALUE"] == false)
		...
		//FIX_1//
	if(array_key_exists("FIX_1", $arResult["PROPERTIES"]) && !$arResult["PROPERTIES"]["FIX_1"]["VALUE"] == false) {
        $sticker .= "<span class='fix'>".GetMessage("CATALOG_ELEMENT_FIX_1")."</span>";
    } else { //Скидка только если не установлен фикс
					...
					<div class="price_buy_detail" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
						...
						<div class="catalog-detail-price" id="<?=$arItemIDs['PRICE'];?>">
							...
							//DETAIL_PRICE//
								...
								<?} else {?>
                                    <?php if (empty($arResult["PROPERTIES"]["FIX_1"]["VALUE"])) : ?>
                                    ...
                                    <?php else :?>
                                        <span class="catalog-detail-item-price-percent">
											<?=GetMessage('CATALOG_ELEMENT_FIX_DISCOUNT')?>
										</span>
                                    <?php endif?>
									<span class="catalog-detail-item-price">
										<span class="catalog-detail-item-price-current">
								...
								<?//OTHER_PRICE//?>
								<?if(is_array($arResult["PRICE_MATRIX_SHOW"]["COLS"]) && count($arResult["PRICE_MATRIX_SHOW"]["COLS"]) > 1) {?>
									<div class="catalog-detail-price-ranges other-price">
...
<?//avg
// добавил расширенную проверку $valQuantity
?>
														...
														<?php
														if ( !empty($arResult['MIN_PRICE']["QUANTITY_FROM"]) ) {
															$valQuantity = $arResult['MIN_PRICE']["QUANTITY_FROM"];
														} else {
															if ($arResult['MIN_PRICE']['MIN_QUANTITY']) {
																$valQuantity = $arResult['MIN_PRICE']["MIN_QUANTITY"];
															} else {
																$valQuantity = 1;
															}
														}
														...
														<input type="text" id="quantity_<?=$arItemIDs['ID']?>" name="quantity" class="quantity" value="<?=$valQuantity?>"/>
					...
					<?if($arResult["COLLECTION"]["THIS"]) {
						...
						<?}
									...
									<p>* Производитель оставляет за собой право без уведомления дилера менять характеристики, внешний вид, комплектацию товара и место его производства.
                                    </p>
            <p>
Указанная информация не является публичной офертой</p>
	...
	<?}
	//DETAIL_KIT_ITEMS//
	if(is_array($arResult["KIT_ITEMS"]) && count($arResult["KIT_ITEMS"]) > 0) {?>
		<div class="kit-items">
