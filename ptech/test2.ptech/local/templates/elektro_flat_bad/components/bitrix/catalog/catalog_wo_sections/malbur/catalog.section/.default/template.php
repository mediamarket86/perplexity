...
<div class="">
<ul class="">
	<?foreach($arResult['BREND'] as $arBrend):?>
	<li><a href="<?=$arBrend['DETAIL_PAGE_URL']?>"><?=$arBrend['NAME']?></a></li>
	<?endforeach;?>
</ul>
</div>
...
<!-- товары -->
	...
	<div class="bx_catalog_top_home news_dop">
		...
		<? endif; ?>
					...
					<p class="art">код <? echo $arItem['PROPERTIES']['KOD_1']['VALUE'] ?></p>
					...
					<? if ($arItem["CATALOG_QUANTITY"]>0) : ?>	
						<a href="<? echo $arItem['ADD_URL']; ?>" data-id="<?= $arItem['ID']; ?>" class="tocart from_fav" onclick="yaCounter34220620.reachGoal('dobavlenie v korzinu'); return true;">В корзину</a>
						<a href="#prod_order" class="fancybox from-preview" data-id="<?= $arItem['ID']; ?>">Купить в 1 клик</a>
					<? else: ?>
						<a href="#zakaz_no_price" class="toorder fancybox" data-id="<?=$arItem['ID'];?>">Под заказ</a>
					<? endif; ?>
...
<script>
	$(document).ready(function() {
		$(".posrel").hover(
		      function () {
			    $(".posrel").removeClass("active"); 
		        $(this).addClass("active");
		        $(".posrel.active .prod_item").show();
	      },
	      function () {
			    $(".posrel").removeClass("active"); 
		        $(".posrel .prod_item").hide();
	      }
	    );
	...
	<table class="prod_list">
		...
		<tfoot>
			...
			foreach ($arResult['ITEMS'] as $key => $arItem) {
				...
				?>
				<tr>
					<td><? echo $arItem['PROPERTIES']['KOD_1']['VALUE']  ?></td>
					<td>
						<div class="posrel">
			...
			<div class="new_price"><?=($minPrice)? round($minPrice['DISCOUNT_VALUE']) : '&nbsp;'; ?></div>
		</td>
...
if ($arParams["DISPLAY_BOTTOM_PAGER"]) {
	...
	<?global $arrFilter;?>
	<?if(empty($_GET['PAGEN_2']) && (empty($arrFilter)|| $arrFilter == 0)){?>
	<div class="text_bottom"><p><?=$arResult["DESCRIPTION"]?></p></div>
	<?}?>
	<?
}
?>
