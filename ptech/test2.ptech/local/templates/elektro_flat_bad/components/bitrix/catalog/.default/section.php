...
$arSelect = array("ID", "XML_ID", "CODE", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "ACTIVE", "GLOBAL_ACTIVE", "PICTURE", "DESCRIPTION", "DEPTH_LEVEL", "SECTION_PAGE_URL", "UF_BANNER", "UF_BANNER_URL", "UF_BACKGROUND_IMAGE", "UF_PREVIEW", "UF_VIEW", "UF_VIEW_COLLECTION", "UF_SECTION_TITLE_H1", "UF_YOUTUBE_BG", "UF_ACTION_BANNER");

$arproductType = array("newproduct", "saleleader", "discount", "FIX_1");

//XML_ID раздела Акции для костылей акций
$actionSectionXmlId = '5abf1775-c6f7-11ed-a50d-a0369f3b7662';
...
if($obCache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_dir)) {
	...
	if($arSection = $rsSections->Fetch()) {
		if(in_array($arSection["CODE"], $arproductType) || $arSection["ACTIVE"] == "Y" || $arSection["GLOBAL_ACTIVE"] == "Y") {
            ...
            $arCurSection["XML_ID"] = $arSection["XML_ID"];
            ...
            /** Банер для акции */
            if (! empty($arSection['UF_ACTION_BANNER'])) {
                $arCurSection['ACTION_BANNER'] = CFile::GetFileArray($arSection['UF_ACTION_BANNER']);
            }
    ...
    /** Костыль для переключения на шаблон акций */
    $sectionChain = CIBlockSection::GetNavChain($arParams['IBLOCK_ID'], $arCurSection['ID'], ['XML_ID'], true);
    $sectionChain = array_column($sectionChain, 'XML_ID');
    $arCurSection['ACTION_SECTION']['IS_ACTION'] = in_array($actionSectionXmlId, $sectionChain);

    if ($arCurSection['ACTION_SECTION']['IS_ACTION']) {
        $sectionObj = new CIBlockSection();

        $arCurSection['ACTION_SECTION']['CHILD_COUNT'] = $sectionObj->GetCount([
            'ACTIVE' => 'Y',
            'GLOBAL_ACTIVE ' => 'Y',
...
/** Костыль для акций */
    /** если нет подразделов выводим до 200 товаров без пагинации */
    if ($arCurSection['ACTION_SECTION']['IS_ACTION'] && 0 == $arCurSection['ACTION_SECTION']['CHILD_COUNT']) {
...
if(!empty($arCurSection)) {
	...
	/** Вывод акционного банера */
	if ($arCurSection['ACTION_SECTION'] && ! empty($arCurSection['ACTION_BANNER']['SRC'])) {?>
		<div class="catalog-item-banner">
				<img src="<?=$arCurSection['ACTION_BANNER']['SRC']?>" width="<?=$arCurSection['ACTION_BANNER']['WIDTH']?>" height="<?=$arCurSection['ACTION_BANNER']['HEIGHT']?>"/>
		</div>
	<?}
    ...
    //Костыль, в разделе акции не выводим товары, только в подразделах!
    if ($arCurSection['XML_ID'] == $actionSectionXmlId) {
        //для хлебных крошек
...
if(!$arCurSection["VIEW_COLLECTION"]) {
	$arAvailableSort = array(
        ...
        "article" => array("PROPERTY_CML2_ARTICLE", "asc"),
...
<?$intSectionID = $APPLICATION->IncludeComponent("bitrix:catalog.section", '',
	array(
        ...
        /** костыль для акций */
        "ACTION_SECTION" => $arCurSection["ACTION_SECTION"],
		),
	$component
);?>
...
<?//DESCRIPTION//
if(!$_REQUEST["PAGEN_1"] || empty($_REQUEST["PAGEN_1"]) || $_REQUEST["PAGEN_1"] <= 1) {?>
	<div class="catalog_preview"><?=(!empty($arCurSection["DESфCRIPTION"]) && empty($pageSeo["SEO_TEXT"]) ? $arCurSection["DESCRIPTION"] : $pageSeo["SEO_TEXT"])?></div>
<?}
