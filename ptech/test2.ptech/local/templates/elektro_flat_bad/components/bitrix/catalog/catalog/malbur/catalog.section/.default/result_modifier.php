...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        $tmp_pic = CFile::ResizeImageGet($productPictures['PICT']['ID'], 
            array('width'=>155, 'height'=>150), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        //PR($productPictures['PICT']);
        //PR($tmp_pic);
        $productPictures['PICT'] = array(
            'SRC' => $tmp_pic['src'],
            'WIDTH' => 155,
            'HEIGHT' => 150
        );
...
if (!empty($arResult['ITEMS']))
{
    ...
    //получение всех оценок по всем товарам
    $ob = CIBlockElement::GetList( Array(), 
        Array('IBLOCK_ID'=>19, 'PROPERTY_TOVARS' => $ids), 
        false, false, 
        Array('PROPERTY_OCENKA', 'PROPERTY_TOVARS') 
    );

    $ocenki = array();    
    while ($res = $ob->GetNext()) {
        $ocenki[] = array(
            't_id' => $res['PROPERTY_TOVARS_VALUE'], 
            'ocenka' => $res['PROPERTY_OCENKA_VALUE'] 
        );
    }
    
    //подсчет средней оценки по каждому товару
	foreach ($arResult['ITEMS'] as $key => $arItem) 
    {
        $sum =0; $i=0;
        foreach($ocenki as $ocenka) 
        if ($ocenka['t_id']==$arItem['ID'])
        {
            $sum += $ocenka['ocenka'];
            $i++;
        }
        ...
        $arResult['ITEMS'][$key]['RATING'] = array (
            'ocenka' => $ocn,
            'count' => $i
        );
        //PR($arResult['ITEMS'][$key]['RATING']);
	}
...
//определение кода из множественного свойства
foreach ($arResult['ITEMS'] as $key => $arItem) 
{
if (isset($arItem['PROPERTIES']['CML2_TRAITS']))
{
	//PR($arItem['PROPERTIES']['CML2_TRAITS']['DESCRIPTION'] );
	//PR($arItem['PROPERTIES']['CML2_TRAITS']['VALUE'] );
	$kod = '';
	foreach ( $arItem['PROPERTIES']['CML2_TRAITS']['DESCRIPTION']  as $i=>$val)
		if ($val=='Код') {
			$kod = $arItem['PROPERTIES']['CML2_TRAITS']['VALUE'][$i]; 
			break;
		}
	$arResult['ITEMS'][$key]['KOD_TOVARA'] = $kod;
}
//if (isset($arItem['PROPERTIES']['PROIZVODITEL']))
{
	//if ($_REQUEST['test'] == 'y') {echo '<pre>';var_export($arResult);echo '</pre>';}
}
}
...
if ($arResult['ID'] > 0)
{
	...
	$rsElements = CIBlockElement::GetList(array(), $arF, false, false, array("PROPERTY_PROIZVODITEL"));
	while($ar = $rsElements->Fetch())
	{
		$arName[$ar['PROPERTY_PROIZVODITEL_VALUE']] = '';
	}
	
	if (array_keys($arName))
	{
		$arF = array (
		  'IBLOCK_ID' => 28,
		  'IBLOCK_ACTIVE' => 'Y',
		  'ACTIVE' => 'Y',
		  'NAME' => array_keys($arName)
		);
		
		$rsElements = CIBlockElement::GetList(array("CODE" => "ASC", "NAME" => "ASC"), $arF, false, false, array("NAME", "DETAIL_PAGE_URL"));
		$rsElements->SetUrlTemplates("/brend/#ELEMENT_CODE#/");
