...
<?php if ('price' == $arParams['TYPE'] && $arParams['ACTION_SECTION']['IS_ACTION'] && 0 == $arParams['ACTION_SECTION']['CHILD_COUNT']) :?>
    <div class="action-section top-action-panel">
        <div class="select-all">
            <span class="multiple-action">
                <label class="action__label active" for="action-section-item-select-all"></label>
                <input class="action__checkbox" id="action-section-item-select-all" type="checkbox" checked>
            </span>
            <span class="select-action select-all-title">
                Выбрать все
            </span>
        </div>
...
<?php //CATALOG//?>
<div id="catalog">	
	<div class="catalog-item-<?=$arParams['TYPE']?>-view <?=$USER->IsAuthorized() ? '' : 'not-auth-view'?>" data-entity="<?=$containerName?>">
        <?php $templateForUser = $USER->isAuthorized() ? '.default' : 'default_not_auth'; ?>
