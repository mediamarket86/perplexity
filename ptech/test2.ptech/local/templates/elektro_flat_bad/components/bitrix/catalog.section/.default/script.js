...
class PriceViewMultipleAction
{
	init() {
		let body = $('body');

		//items checkbox change
		body.on('change', '.catalog-item-list.list-view .multiple-action.select-item .action__checkbox', BX.proxy(this.itemCheckboxChange, this));
		...
		//add selected button
		body.on('click',  '.action-section.top-action-panel .add-selected .add-selected__link', BX.proxy(this.addSelected, this));
	}
	...
	selectAllCheckboxChange(elm) {
		...
		$('.catalog-item-list.list-view .multiple-action.select-item .action__checkbox').each(function(index, catItem) {
	...
	setSelectedCount() {
		var count = 0;
		$('.catalog-item-list.list-view .multiple-action.select-item .action__checkbox').each(function(index, elm) {
			if (elm.checked) {
				count++;
			}
		});
...
BX.ready(function () {
	let actionControll = new PriceViewMultipleAction();
	actionControll.init();
});