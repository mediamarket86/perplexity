...
<div class="filtered-items">
	...
	<div class="catalog-item-cards">
		<?foreach($arResult["ITEMS"] as $key => $arElement) {				
			...
			//PRICE_MATRIX//
			if(isset($arElement["PRICE_MATRIX_SHOW"]["COLS"]) &&  count($arElement["PRICE_MATRIX_SHOW"]["COLS"]) > 1 && empty($arElement["CURRENT_DISCOUNT"]["ACTIVE_TO"])) {
				$class = " item-pm";
			}
			if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) {
...
<script type="text/javascript">
	BX.ready(function() {
		...
		<?foreach($arResult["ITEMS"] as $key => $arElement) {
			...
			var <?=$strObName?> = new JCCatalogPopertySearchProducts(<?=CUtil::PhpToJSObject($arJSParams, false, true);?>);
		<?}?>
	});
</script>
