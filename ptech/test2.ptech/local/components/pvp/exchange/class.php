    ...
    protected function initController(): void
{
    ...
    if ($this->arParams['DEBUG_MODE'] === 'Y') {
        $this->errorManager->addError("DEBUG: Initializing controller");
        $this->errorManager->addError("DEBUG: Controller name: " . $componentVariables['CONTROLLER']);
        $this->errorManager->addError("DEBUG: Method name: " . $componentVariables['METHOD']);
    }
    ...
    if ($controllerClass = $this->findControllerClass($componentVariables['CONTROLLER'])) {
        ...
        if ($this->checkMethod($controller, $method)) {
            ...
            if ($this->arParams['DEBUG_MODE'] === 'Y') {
                $this->errorManager->addError("DEBUG: Controller initialized successfully");
            }
            return;
        }
    }
    ...
    if ($this->arParams['DEBUG_MODE'] === 'Y') {
        $this->errorManager->addError("DEBUG: Failed to initialize controller");
    }
    $this->setNotFound();
}
