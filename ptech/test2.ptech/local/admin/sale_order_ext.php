...
if (!empty($orderList) && is_array($orderList))
{
    ...
    foreach ($orderList as $orderId => $arOrder)
    {
        ...
        if($bNeedBasket)
        {
            ...
            foreach ($arBasketItems as $arItem)
            {
                    ...
                    if (!empty($basketPropertyList[$arItem["ID"]]) && is_array($basketPropertyList[$arItem["ID"]]))
                    {
                        foreach ($basketPropertyList[$arItem["ID"]] as $prop)
                        {
                            ...
                            if ($prop["CODE"] == 'CML2_ARTICLE') {
                              $fieldArtic .= htmlspecialcharsbx($prop["VALUE"]);
                            }
                        }
                    }
                }
                if($fieldArtic == '' || $fieldArtic[strlen($fieldArtic) - 1] == '>') {
                    $arSelect = Array('PROPERTY_CML2_ARTICLE');
                    $arFilter = Array("ID" => $arItem['PRODUCT_ID']);
                    $res = CIBlockElement::GetList(false, $arFilter, false, false, $arSelect);
                    while ($ob = $res->Fetch()) {
                        $fieldArtic .= $ob['PROPERTY_CML2_ARTICLE_VALUE'];
                    }
                }
                ...
                if(strlen($arItem["QUANTITY"]) > 0)
                    $fieldQuantity .= intval(htmlspecialcharsbx(Sale\BasketItem::formatQuantity($arItem["QUANTITY"])));
                else
                    $fieldQuantity .= "<br />";
        ...
        $row->AddField("PROP_ARTICUL", $fieldArtic);
