...
public function getNew(): void
{
    if ($this->arParams['DEBUG_MODE'] === 'Y') {
        $this->errorManager->addError("DEBUG: Entering getNew method");
    }
    ...
    if ($this->arParams['DEBUG_MODE'] === 'Y') {
        $this->errorManager->addError("DEBUG: Requested page: " . $page);
    }
    ...
    if ($this->arParams['DEBUG_MODE'] === 'Y') {
        $this->errorManager->addError("DEBUG: Applied filter: " . print_r($filter, true));
    }

    $exchangeComponent->includeComponentPage('shop.sectionFiltred');

    if ($this->arParams['DEBUG_MODE'] === 'Y') {
        $this->errorManager->addError("DEBUG: Exiting getNew method");
    }
}
