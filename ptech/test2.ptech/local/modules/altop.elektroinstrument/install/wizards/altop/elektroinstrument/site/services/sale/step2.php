...
if(COption::GetOptionString("elektroinstrument", "wizard_installed", "N", WIZARD_SITE_ID) != "Y") {
	$locationGroupID = 0;
	$arLocationArr = Array();
	...
	if(!$dlvBySiteExist) {
		$deliveryItems[] = array(
			...
			"LOGOTIP" => WIZARD_SERVICE_RELATIVE_PATH ."/images/delivery/courier.jpg",
			"CONFIG" => array(
				"MAIN" => array(
					"PRICE" => ($bRus ? "500" : "30"),
					"CURRENCY" => $defCurrency,
		...
		$deliveryItems[] = array(
			...
			"LOGOTIP" => WIZARD_SERVICE_RELATIVE_PATH ."/images/delivery/pickup.jpg",
			"CONFIG" => array(
				"MAIN" => array(
					"PRICE" => 0,
					"CURRENCY" => $defCurrency,
...
if(CModule::IncludeModule("subscribe")) {
	...
	if(!$rsRubric->Fetch()) {
		...
		$arFields = Array(
			...
			"FROM_FIELD" => COption::GetOptionString("main", "email_from", "noreply@ptech"),
			"LAST_EXECUTED"	=> ConvertTimeStamp(false, "FULL"), 
		);
		$obRubric = new CRubric;
		$ID = $obRubric->Add($arFields);
	}
	COption::SetOptionString("subscribe", "subscribe_section", "#SITE_DIR#personal/mailings/");
}
...
if(IntVal($userGroupID) > 0) {
	...
	WizardServices::SetFilePermission(Array($siteID, $SiteDir."/reviews/"), Array($userGroupID => "W"));
	WizardServices::SetFilePermission(Array($siteID, $SiteDir."/vendors/"), Array($userGroupID => "W"));	
}
COption::SetOptionString("elektroinstrument", "wizard_installed", "Y", false, WIZARD_SITE_ID);?>