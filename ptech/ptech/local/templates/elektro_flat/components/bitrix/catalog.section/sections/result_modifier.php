...
if(count($arResult["SECTIONS"]) > 0) {
	$rsSections = CIBlockSection::GetList(
		...
		false,
		array("ID", "IBLOCK_ID", "SORT", "NAME", "PICTURE")
	);
	while($arSection = $rsSections->GetNext()) {		
		if($arResult["SECTIONS"][$arSection["ID"]]) {
			...
			if (! empty($arSection['PICTURE'])) {
                $arResult["SECTIONS"][$arSection["ID"]]["ICON"] = CFile::GetFileArray($arSection['PICTURE']);
            }
		}
	}
...
foreach($arResult["SECTIONS"] as $sectionKey => $section) {
    foreach ($section['ITEMS'] as $itemKey => $item ) {
        $item['MIN_PRICE']['RATIO_PRICE'] = round($item['MIN_PRICE']['RATIO_PRICE'], 2);
        ...
        if ($elementMinPrice && $item['ITEM_PRICES'][$item['ITEM_PRICE_SELECTED']]['PRICE'] < $elementMinPrice) {
            $arResult["SECTIONS"][$sectionKey]['ITEMS'][$itemKey]['MIN_PRICE']['RATIO_PRICE'] = round($elementMinPrice, 2);
            $arResult["SECTIONS"][$sectionKey]['ITEMS'][$itemKey]['MIN_PRICE']['PRINT_RATIO_DISCOUNT'] = CurrencyFormat($item['ITEM_PRICES'][$item['ITEM_PRICE_SELECTED']]['BASE_PRICE'] - $elementMinPrice, $item['ITEM_PRICES'][$item['ITEM_PRICE_SELECTED']]['CURRENCY']);
            $arResult["SECTIONS"][$sectionKey]['ITEMS'][$itemKey]['MIN_PRICE']['PRINT_RATIO_PRICE'] = CurrencyFormat($elementMinPrice, $item['ITEM_PRICES'][$item['ITEM_PRICE_SELECTED']]['CURRENCY']);
        }
    }
}
//END_SECTIONS//?>