...
$template = $USER->IsAuthorized() ? '.default' : 'default_not_auth';
//ELEMENT//?>
<?$ElementID = $APPLICATION->IncludeComponent("bitrix:catalog.element", $template,
	array(
...
if(!empty($arCurElement)) {
	...
	if(!$arCurElement["IS_COLLECTION"] && $arParams["RELATED_PRODUCTS_SHOW"] !== "N") {	
        ...
        $template = $USER->IsAuthorized() ? 'filtered' : 'filtered_not_auth';
