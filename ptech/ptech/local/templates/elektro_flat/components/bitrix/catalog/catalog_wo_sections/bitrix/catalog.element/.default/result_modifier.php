...
//определение средней оценки товара
$ob = CIBlockElement::GetList( Array(), Array('IBLOCK_ID'=>19, 'ACTIVE'=>'Y', 'PROPERTY_TOVARS' => $arResult['ID']), false, false, Array('PROPERTY_OCENKA') );
$sum =0; $i=0;
while ($res = $ob->GetNext()) {
    $sum += $res['PROPERTY_OCENKA_VALUE'];
    $i++;
}
$arResult['RATINGS_COUNT'] = $i;
$arResult['RATING'] = round($sum / $i);
if ($arResult['RATING']<0) $arResult['RATING'] = 0;
if ($arResult['RATING']>5) $arResult['RATING'] = 5;

 

$tmp_pic = CFile::ResizeImageGet($arResult['DETAIL_PICTURE'], 
    array('width'=>276, 'height'=>260), BX_RESIZE_IMAGE_PROPORTIONAL, true);
$arResult['DETAIL_PICTURE_SMALL'] = array(
    'SRC' => $tmp_pic['src'],
    'WIDTH' => $tmp_pic['width'],
    'HEIGHT' => $tmp_pic['height'],
);
//PR($arResult['DETAIL_PICTURE_SMALL']);
//PR($arResult,1);
//PR($arResult['CATALOG_PRICE_2']);
//PR($arResult['PROPERTIES']['PROIZVODITEL']);
//PR($arResult['PROPERTIES']['FILES']);

//для рандомного выбора товаров
$arResult['TOVAR_ID_RANGE'] = array(
    'MIN' => 1472,
    'MAX' => 2219
);
...
//******Similar Products********//
$arResult["DOP_TOVAR"] =array();
$idSimProduct =array();
$resIdSimProducts = array();
$kolSimProducts = 4;
$arSelect = Array("ID");
$arFilter = Array("SECTION_ID"=>$arResult["IBLOCK_SECTION_ID"], "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y","!ID"=>$arResult["ID"]);
$res = CIBlockElement::GetList(Array("ID"=>"ASC"), $arFilter, false, flase, $arSelect);
    while($ob = $res->GetNextElement())
    {
        $arFields = $ob->GetFields();
        $idSimProduct[] = (int)$arFields["ID"];
    }
$idhash = md5($arResult["ID"]);
if(count($idSimProduct) > $kolSimProducts) {
    while (true) {
        for ($i = 26; $i--; ) {
          $idElem = $idSimProduct[hexdec(substr($idhash, $i, 6)) % count($idSimProduct)];
          if (!in_array($idElem, $resIdSimProducts)) $resIdSimProducts[] = $idElem;
          if (count($resIdSimProducts) == $kolSimProducts) break 2;
        }
        $idhash = md5($idhash);
    }
} else {
    $resIdSimProducts = $idSimProduct;
    shuffle($resIdSimProducts);
}
$arSelect = Array("ID","NAME", "PREVIEW_PICTURE", "DETAIL_PICTURE", "DETAIL_PAGE_URL");
$res = CIBlockElement::GetList(Array(), array("ID"=>$resIdSimProducts), false, flase, $arSelect);
    while($ob = $res->GetNextElement())
    {
        $arFields = $ob->GetFields();
        $simProduct[(int)$arFields["ID"]]["NAME"] = $arFields["NAME"];
        if($arFields["PREVIEW_PICTURE"]) {
            $simProduct[(int)$arFields["ID"]]["PREVIEW_PICTURE"] = CFile::GetPath($arFields["PREVIEW_PICTURE"]);
        } else {
            $simProduct[(int)$arFields["ID"]]["DETAIL_PICTURE"] = CFile::GetPath($arFields["DETAIL_PICTURE"]);
        }
        $simProduct[(int)$arFields["ID"]]["DETAIL_PAGE_URL"] = $arFields["DETAIL_PAGE_URL"];
    }
$arResult["SIMILAR_TOVAR"] = $simProduct;
