...
<div class="kabinet" id="kabinet">
	<?$frame = $this->createFrame("kabinet")->begin("");?>
		...
		<?if(!$USER->IsAuthorized()):?>
			<a class="login_anch" href="javascript:void(0)" title="<?=GetMessage("LOGIN")?>"><i class="svg-icon svg-user"></i><span><?=GetMessage("LOGIN")?></span><i class="svg-icon svg-sign-in"></i></a>
					...
					<div class="fields">
                        <form name="form_auth" id="ajax-login-form" method="post" target="_top" action="<?=$this->GetFolder()?>/ajax.php">
							...
							<div class="field" style="margin:0px;">
								<a class="btn_buy apuo reg" href="<?=SITE_DIR?>register/" rel="nofollow"><?=GetMessage("AUTH_REGISTRATION")?></a>
							</div>
						</form>
						<script type="text/javascript">
			...
			<a class="register" href="<?=SITE_DIR?>register/" title="<?=GetMessage("REGISTRATION")?>" rel="nofollow"><i class="svg-icon svg-register"></i><span><?=GetMessage("REGISTRATION")?></span></a>
		<?else:?>
