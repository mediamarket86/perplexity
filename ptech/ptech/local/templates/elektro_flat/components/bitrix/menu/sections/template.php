		...
		if($arItem["DEPTH_LEVEL"] == 1) {
			if($arItem["IS_PARENT"]) {?>
				<li class="parent<?=($arItem['SELECTED'] ? ' selected' : '')?>">
					<a class="catalog-link" href="<?=$arItem['LINK']?>">
                        <span class="catalog-link-icon">
                            <?php if(is_array($arItem["UF_SECTION_ICON"])) {?>
                            <img class="catalog-link-icon__img" src="<?=$arItem['UF_SECTION_ICON']['SRC']?>" alt="<?=$arItem['TEXT']?>" title="<?=$arItem['TEXT']?>" />
                            <?} else {?>
                                <img class="catalog-link-icon__img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.jpg" alt="<?=$arItem['TEXT']?>" title="<?=$arItem['TEXT']?>" />
                            <?}?>
                        </span>
                        <span class="catalog-link-text"><?=$arItem["TEXT"].($arSetting["CATALOG_LOCATION"]["VALUE"] == "LEFT" ? "<span class='arrow'></span>" : "")?></span>
			...
			<?} else {?>
				...
				<li<?=($arItem["SELECTED"] ? " class='selected'" : "")?>>
                    ...
                    <span class="catalog-link-icon">
                        ...
                        <span class="catalog-link-text"><?=$arItem["TEXT"]?></span>
                    </a>
				</li>
			<?}
