...
//	);
    ...
    if (\Bitrix\Main\Loader::includeModule('pvp.favorites')) {
        ...
        $favoritesCount = $USER->IsAuthorized() ? $favorites->getCount(\Bitrix\Main\Engine\CurrentUser::get()->getId()) : 0;
...
if($customPagesList) {
    foreach($customPagesList as $page) {
        $availablePages[] = array(
            ...
            "class" => empty($page[3]) ? null : $page[3],
        );
    }
}
	...
	<div class="sale-personal-section__list">		
		<?foreach($availablePages as $blockElement) {?>
			<div class="sale-personal-section__item">
				<a class="sale-personal-section__item-link <?=empty($blockElement['class']) ? '' : $blockElement['class']?>" href="<?=htmlspecialcharsbx($blockElement['path'])?>">
