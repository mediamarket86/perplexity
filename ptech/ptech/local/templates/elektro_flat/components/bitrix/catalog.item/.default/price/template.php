...
<div class="catalog-item-list list-view">
    <div class="left group">
        <?php if ($arParams['ACTION_SECTION']['IS_ACTION'] && 0 == $arParams['ACTION_SECTION']['CHILD_COUNT']) : ?>
            <div class="multiple-action select-item">
                <label class="action__label active" for="multiple-action-controls-item-<?=$itemIds['ID']?>"></label>
                <input class="action__checkbox" id="multiple-action-controls-item-<?=$itemIds['ID']?>" type="checkbox" checked data-bx-item-id="<?=$itemIds['ID']?>">
            </div>
        <?php endif; ?>
        ...
        <div class="picture">
            ...
            <a class="picture-link" href="<?=$arElement['DETAIL_PAGE_URL']?>">
                <?if(is_array($arElement["PREVIEW_PICTURE"])) {?>
                    <img class="picture-link__img" src="<?=$arElement['PREVIEW_PICTURE']['SRC']?>" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
                <?} else {?>
                    <img class="picture-link__img" src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.jpg" alt="<?=$strAlt?>" title="<?=$strTitle?>" />
                <?}?>
            </a>
        </div>
        <?//ITEM_ARTICLE//?>
        <?php if (! empty($arElement['PROPERTIES']['CML2_ARTICLE']['VALUE'])) : ?>
            <div class="article">
    ...
    <div class="right group">
        ...
        <div class="item-price<?=$class?>" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
               <?php if($arElement["MIN_PRICE"]["RATIO_PRICE"] <= 0) {?>
                ...
                <?php } else { ?>
                    ...
                    <?}
                    if(is_array($arElement["PRICE_MATRIX"]["MATRIX"]) && count($arElement["PRICE_MATRIX"]["MATRIX"]) > 1 && count($arElement["ITEM_QUANTITY_RANGES"]) <= 1) {?>
                        <span class="catalog-item-price-ranges-wrap">
							<a id="<?=$itemIds['PRICE_RANGES_BTN']?>" class="catalog-item-price-ranges" href="javascript:void(0);"><i class="fa fa-question-circle-o"></i></a>
						</span>
                    <?}
                    if($arSetting["REFERENCE_PRICE"] == "Y" && !empty($arSetting["REFERENCE_PRICE_COEF"])) {?>
                        <span class="catalog-item-price-reference">
							<?=CCurrencyLang::CurrencyFormat($arElement["MIN_PRICE"]["RATIO_PRICE"] * $arSetting["REFERENCE_PRICE_COEF"], $arElement["MIN_PRICE"]["CURRENCY"], true);?>
						</span>
                    <?}?>
				</span>
                <?}?>
...
<?php
/**
 * Дальще заккоментирован оригинальный шаблон
 */
/*
?>
