...
class CBitrixBasketComponent extends CBitrixComponent
{
    ...
    protected function refreshAndCorrectRatio()
    {
        ...
        if ($this->arParams['CORRECT_RATIO'] === 'N')
        {
    ...
    protected function processBasketItem(Sale\BasketItem $item)
    {
        ...
        $basketItem['PRICE'] = PriceMaths::roundPrecision($basketItem['PRICE'] < $elementMinPrice ? $elementMinPrice : $basketItem['PRICE']);
        $basketItem['PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['PRICE'], $basketItem['CURRENCY'], true);
    ...
    protected function isNeedBasketUpdateEvent()
    {
        ...
        foreach ($basket as $basketItem)
        {
            ...
            {
                $oneItemPrice = $basketItem->getFinalPrice() / (float)$basketItem->getField('QUANTITY');
                $basketPrice += $oneItemPrice < $elementMinPrice ? $elementMinPrice * (float)$basketItem->getField('QUANTITY') : $basketItem->getFinalPrice();
            }
        }
